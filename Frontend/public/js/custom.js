...
$(function(){
	...
	tabsClick.on('click', function(e)  {
		...
		if(currentAttrValue=="#favoritelist"){
			document.getElementById('value').innerText= 'Favorite List';
		}
		else if(currentAttrValue=="#profile"){
			document.getElementById('value').innerText= 'Profile';
		}
	...
	// 	console.log(searchParams.get('page')); // price_descending
	// 	localStorage.setItem(searchParams.get('page'),"active");
	// });
	
	$(document).ready(function (event) {
		let searchParams = new URLSearchParams(window.location.search).get('page');
		if(!searchParams){searchParams='1';}
		searchParams = "#page" + searchParams;
		$(searchParams).addClass('active').siblings().removeClass('active');
	  });
	$(document).ready(function (event) {
		let searchParams = new URLSearchParams(window.location.search).get('number');
		searchParams = "#number" + searchParams;
		$(searchParams).addClass('active').siblings().removeClass('active');
	  });
	$(document).ready(function (event) {
		let searchParams = new URLSearchParams(window.location.search).get('sort').replaceAll(' ','');
		console.log(searchParams)
		searchParams = "#sort" + searchParams;
		$(searchParams).addClass('active').siblings().removeClass('active');
	  });
	// window.addEventListener ? 
	// window.addEventListener("load",asdf,false) : 
	// window.attachEvent && window.attachEvent("onload",asdf);
